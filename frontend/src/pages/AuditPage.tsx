...
export default function AuditPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const logsData = await apiClient
        .getAuditLogs({
          agent_id: filterAgent || undefined,
          policy_decision:
            filterDecision !== "All" ? filterDecision : undefined,
          limit: 100,
        })
        .catch(() => null);
      ...
      if (statsData) {
        setStats({
          total: statsData.total_events ?? statsData.total ?? 0,
          allow_count: statsData.allow_count ?? 0,
          deny_count: statsData.deny_count ?? 0,
          by_agent: statsData.by_agent ?? {},
        });
      } else {
        setStats({
          total: 28,
          ...
          deny_count: 4,
          by_agent: {
            "cybersecurity-log-analyst": 18,
            "read-pdf-invoices-agent": 10,
          },
        });
      }
        ...
        const defaultRecords: AuditRecord[] = [
          {
            ...
            policy_reason:
              "Platform Hard Deny: Target path matches protected system files",
            latency_ms: 2,
          ...
          },
          {
            ...
            policy_reason:
              "Matched Cedar permit rule #1 for cybersecurity-log-analyst",
            latency_ms: 3,
          ...
          },
          {
            ...
            policy_decision: "DENY",
            policy_reason:
              "Implicit Deny: Confidential agent forbidden from external endpoints",
            latency_ms: 1,
          ...
          },
          {
            ...
            policy_decision: "ALLOW",
            policy_reason:
              "Matched Cedar permit rule for read-pdf-invoices-agent",
            latency_ms: 4,
  ...
  const filteredRecords = records.filter((r) => {
    if (
      filterAgent &&
      !r.agent_id.toLowerCase().includes(filterAgent.toLowerCase())
    )
      return false;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b border-slate-200">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Immutable log stream. Every sandbox tool invocation and
            authorization decision is verified against Cedar PDP before
            execution.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl border border-emerald-200/80 bg-gradient-to-b from-white to-emerald-50/20 shadow-2xs space-y-2">
          ...
          <p className="text-[11px] text-emerald-600/80 font-mono">
            Validated against permit rules
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-rose-200/80 bg-gradient-to-b from-white to-rose-50/20 shadow-2xs space-y-2">
          ...
          <p className="text-[11px] text-rose-600/80 font-mono">
            Hard denies and out-of-scope calls
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-purple-200/80 bg-gradient-to-b from-white to-purple-50/20 shadow-2xs space-y-2">
          ...
          <div className="text-2xl font-bold text-purple-900 font-mono">
            {typeof stats?.by_agent === "object"
              ? Object.keys(stats.by_agent).length
              : 2}
          </div>
          <p className="text-[11px] text-purple-600/80 font-mono">
            Independent MicroVM sandboxes
          </p>
        </div>
      </div>
              ...
              >
                ...
                <div className="space-y-1.5 bg-slate-50 p-3.5 rounded-lg border border-slate-200">
                  <div className="text-slate-500">
                    Decision:
                    <span
                      className={`ml-2 font-bold px-2 py-0.5 rounded text-[10px] ${
                  ...
                  <div className="text-slate-800 bg-white p-2.5 rounded border border-slate-200 text-[11px] leading-relaxed font-sans">
                    {r.policy_reason ||
                      "Evaluated against active Cedar policy set"}
                  </div>
                </div>
              </div>
            ))}
